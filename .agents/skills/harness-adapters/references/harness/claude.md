...
| Permissions | `--permission-mode auto` by default in this fork, or `--dangerously-skip-permissions` when `config/claude-permission-mode` is `bypass`; the `auto` shape verified on 2.1.269, and `../../../../../docs/configuration.md` "Claude permission mode" owns the file. |
