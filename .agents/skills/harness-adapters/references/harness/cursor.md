...
| Autonomy | `--auto-review --sandbox enabled`, this fork's hardened posture: commands run inside Cursor's own sandbox while auto-review keeps watching, replacing the former blanket `--yolo` (`--force` alias, footer `Run Everything`) bypass. |
| Trust | `--trust` suppresses the dialog; it is also what loads the project hooks, and every task has a fresh path. |
