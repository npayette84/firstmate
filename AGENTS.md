...
config/claude-permission-mode  optional one-token permission posture for every Claude worker launch: absent or "auto" launches with --permission-mode auto (this fork's hardened default), "bypass" restores --dangerously-skip-permissions (the upstream default); LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Claude permission mode"
