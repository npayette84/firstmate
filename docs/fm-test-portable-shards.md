...
The 27 scripts that were still unmeasured on 2026-09-17 took their first hint from the completed `FM_TEST_END duration_ms=` markers of the five portable serial shards of [run 35259438775](https://github.com/kunchenguid/firstmate/actions/runs/35259438775); each of those scripts finished with `exit=0` in that run even though two shards failed on other scripts.
