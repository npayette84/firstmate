...
#   secondmate, and relaunch) carries. This fork defaults to `auto`, launching
#   with `--permission-mode auto`, Claude Code's classifier-reviewed mode, so
#   workers run under claude's own approval controls; `bypass` opts back into
#   `--dangerously-skip-permissions`, the upstream default, for a captain who
#   wants full bypass. Every other part of the claude launch
...
launch_template() {
  ...
  case "$harness" in
    ...
    # selects (header above): --permission-mode auto by default in this fork,
    # or --dangerously-skip-permissions for a captain who opts back into
    # bypass mode.
