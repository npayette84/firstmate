...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  expected="env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --permission-mode auto --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}' \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/launch-brief.md')\""
...
# must both produce the classifier-reviewed launch byte-for-byte, `bypass`
# swaps in the upstream bypass flag, and any other token refuses before
...
test_claude_permission_mode_bypass_restores_bypass_launch() {
  ...
  [ "$launch" = "$expected" ] || fail "explicit bypass did not restore the bypass launch"$'\n'"expected: $expected"$'\n'"actual:   $launch"
  assert_not_contains "$launch" "--permission-mode auto" "bypass launch must not carry the default auto mode"
  pass "config/claude-permission-mode=bypass restores the upstream bypass launch"
}
